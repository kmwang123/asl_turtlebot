...
class AStar(object):
    ...
    def reset(self, x_init, x_goal):

        #resets the class for run with new start and end goal
    ...
    def is_free(self, x):
        ...
        """
        ########## Code starts here ##########
        in_bounds = all(x >= lo and x <= hi for x, lo, hi in zip(x, self.statespace_lo, self.statespace_hi))
    ...
    def get_neighbors(self, x):
        ...
        neighbors = []
        ########## Code starts here ##########
        headings = np.linspace(0, 2*np.pi, num=8, endpoint=False)
        for h in headings:
            x1 = x[0] + self.resolution * np.cos(h)
            x2 = x[1] + self.resolution * np.sin(h)
            neighbor = self.snap_to_grid((x1, x2))
...
#        self.path = self.reconstruct_path()
    ...
    def solve(self):
        ...
        """
        ########## Code starts here ##########
        self.open_set.add(self.x_init)  # 1
        self.cost_to_arrive[self.x_init] = 0  # 3
        self.est_cost_through[self.x_init] = self.distance(self.x_init, self.x_goal)  # 4

        while len(self.open_set) > 0:  # 5
#            print('open set', self.open_set)
#            print('est cost thru', self.est_cost_through)

            open_set_costs = {x : self.est_cost_through[x] for x in self.open_set} # 6
            x_current = min(open_set_costs, key=open_set_costs.get) # 6


            if x_current == self.x_goal:  # 7
                self.path = self.reconstruct_path()
                return self.path  # 8

            self.open_set.remove(x_current)  # 10
            self.closed_set.add(x_current)  # 11

            for neighbor in self.get_neighbors(x_current):  # 12

                if neighbor in self.closed_set:  # 13
                    continue  #14

                tentative_CTA = self.cost_to_arrive[x_current] + self.distance(x_current, neighbor)  # 16


                if neighbor not in self.open_set:  # 17
                    self.open_set.add(neighbor)  # 18
                elif tentative_CTA > self.cost_to_arrive[neighbor]:
                    continue

                self.came_from[neighbor] = x_current  # 22
                self.cost_to_arrive[neighbor] = tentative_CTA  # 23
                self.est_cost_through[neighbor] = tentative_CTA + self.distance(self.x_goal, neighbor)
